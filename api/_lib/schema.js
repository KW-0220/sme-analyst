...
export const Extraction = z.object({
  ...
  })).describe("只包括第一個戶口、第一種幣種的交易；期初／期末結餘行不是交易"),
...
export const GEMINI_SCHEMA = S("OBJECT", {
  properties: {
    ...
    accounts: S("ARRAY", { items: S("OBJECT", { properties: { bank_name: S("STRING"), account_type: S("STRING"), masked_number: S("STRING"), currency: S("STRING") }, required: ["bank_name", "account_type", "masked_number", "currency"] }) }),
    statement_period: S("OBJECT", { nullable: true, properties: { start: S("STRING"), end: S("STRING") }, required: ["start", "end"] }),
    opening_balance: S("NUMBER", { nullable: true }),
    closing_balance: S("NUMBER", { nullable: true }),
    transactions: S("ARRAY", { items: S("OBJECT", {
      properties: {
        date: S("STRING"), description: S("STRING"), amount: S("NUMBER"), balance_after: S("NUMBER", { nullable: true }), page: S("INTEGER"),
        category: S("STRING", { enum: CATEGORIES }), needs_confirmation: S("BOOLEAN"), confirmation_reason: S("STRING", { nullable: true })
      },
      required: ["date", "description", "amount", "balance_after", "page", "category", "needs_confirmation", "confirmation_reason"]
    }) }),
  ...
  required: ["is_bank_statement", "not_statement_reason", "accounts", "statement_period", "opening_balance", "closing_balance", "transactions", "pages_total", "pages_readable", "unreadable_notes", "multiple_currencies"]
});
