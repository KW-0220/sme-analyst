...
(function () {
  ...
  const VIEWS = {
    ...
    choose: (reports) => '<span class="eyebrow">完成</span><h1 style="font-size:1.5rem">文件包含 ' + reports.length + ' 個戶口，請選擇先看哪一個</h1>' + fileLine() + stepsHtml("done") +
      '<p class="ink2">每個戶口及幣種各自一份摘要，不同幣種不會相加。閱讀報告時可隨時切換。</p>' +
      '<div class="btn-row" style="flex-direction:column;align-items:stretch">' + reports.map(r => r.empty
        ? '<div class="notice"><span class="notice__icon" aria-hidden="true" style="background:var(--surface-3)">–</span><p><strong>' + esc(r.accountLabel) + '</strong><br><span class="small muted">' + esc(r.note) + '</span></p></div>'
        : '<button type="button" class="btn btn--secondary" data-choose="' + r.accountIndex + '" style="justify-content:space-between"><span>' + esc(r.accountLabel) + '</span><span class="small muted">' + r.transactions.length + ' 筆交易</span></button>').join("") + "</div>",
  ...
  function render(stage, arg) {
    ...
    card.querySelectorAll("[data-choose]").forEach(b => b.addEventListener("click", () => {
      window.SME_API.selectReport(jobId, Number(b.getAttribute("data-choose")));
      track("account_selected");
  ...
  async function run() {
    ...
    if (!file) {
      const existing = window.SME_API.getReports(jobId);
      if (existing.length > 1) { render("choose", existing); return; }
    ...
    const reports = Array.isArray(result.reports) ? result.reports : [r];
    if (reports.length > 1) render("choose", reports);
    else if (r.partial) render("partial", r.document.notes);
    else render("done");
  }
